...
su_info::su_info(int uid) : uid(uid), eval_uid(-1), access(DEFAULT_SU_ACCESS), mgr_uid(-1),
...
void su_daemon_handler(int client, const sock_cred *cred)
{
    ...
    if (ctx.info->access.policy == QUERY)
    {
        ...
        else
        {
            ...
            // set access to ALLOW manually
            ctx.info->access.policy = ALLOW;
            close(fd);
        }
    }
    else ctx.info->access.policy = ALLOW;
